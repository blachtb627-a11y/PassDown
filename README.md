...
- **Home feed** — a bold deep-green header banner (logo, "Passed Down" wordmark, notifications), a row of Circle filter chips (jump straight to what a specific circle has shared), a floating search bar, a **Public / Following** tab switcher, and two compact filter dropdowns — **Cuisine** (every country, with its own search box) and **Meal Type** — replacing what used to be a wrapping row of chip bubbles. Public shows every recipe you can see (the old default feed); Following narrows that down to just the people you follow. Below that, a single-column card feed: each card leads with the poster's name ("passed this down"), the photo with title overlaid, a cuisine badge, diet/occasion/meal-type tags, and like/comment/share/save/circle actions. Tapping the comment icon jumps straight to that recipe's comments and focuses the input.
...
- **Search** — people only: search by name/username, or browse a "Suggested for you" list of accounts you don't already follow, with a follow button on every result. Following someone sends a request rather than following instantly (see Follow requests below); a **Follow Requests** section appears at the top of this tab whenever someone's asked to follow you, with Accept/Decline on each. Recipe search/filtering lives on the Home feed instead (see above), rather than being split across two tabs.
...
- `comments`, `made_this_posts`, `likes`, `saves`, `collections`, `collection_recipes`, `shopping_list_items`.
- `follows` — `status` is `'pending'` until the followee accepts it, then `'accepted'` (see Follow requests below); `follower_id`/`followee_id` reference `profiles`.
...
Every table has row-level security: published recipes and social data (comments, likes, accepted follows) are readable by everyone, but people can only write their own rows; saves, collections, the shopping list, and a still-pending follow request are private to the account(s) involved.

## Follow requests

Following someone sends a request instead of following instantly — like a private-account model on Instagram/TikTok, applied to every account rather than an opt-in setting. A pending request only counts as a real follow (for follower/following counts, the Home feed's Following tab, and seeing that person's private recipes) once they accept it.

- The **Follow** button on a profile, a recipe's author strip, or a Search result cycles through **Follow → Requested → Following**: tapping it sends a request, sends it again to cancel a still-pending one, or unfollows an accepted one.
- Incoming requests show up in a **Follow Requests** section at the top of the Search tab, with Accept/Decline on each — see the Search bullet above.
- `supabase/migrations/..._add_follow_requests.sql` adds the `status` column and replaces the table's row-level security to match: sending a request only succeeds as yourself and always starts pending; only the person being followed can accept their own incoming request; either side can delete a row (unfollow, cancel a request, or decline one); a pending request is only visible to the two people it involves. It also updates the `recipes` SELECT policy so a private recipe requires an *accepted* follow, not just a pending request. Existing follows from before this migration are grandfathered in as accepted, so nobody's existing followers disappear.

To apply it: open the Supabase Dashboard → **SQL Editor** → **New query**, paste in the full contents of `supabase/migrations/20260830220000_add_follow_requests.sql`, and click **Run**. No Edge Function or secret is involved — this is a plain schema/RLS change.
